document.addEventListener("DOMContentLoaded", function () {
    const chartConfigs = {
    "genderChart": ["/gender_distribution", "pie", "Gender Distribution", ["#36A2EB", "#fc4293"]],
        "topStatesChart": ["/top10_states", "bar", "Top 10 States", "#FFF56D", "horizontal"],
        "ageChart": ["/age_distribution", "line", "Age Distribution", "#FFD700"],
        ...
        "subAgenciesChart": ["/top5_sub_agencies", "bar", "Top 5 Sub Agencies", "#FF6384","horizontal"],
            ...
            .then(data => {
                ...
                let newChart = new Chart(canvas, {
                    ...
                    options: {
                        ...
                        rotation: -90,  // <<< Force chart to start at top with first label
                        plugins: {
                            legend: {
                                labels: { color: "#ffffff" }
                            }
    ...
    function openChartModal(chartId, title, description) {
        ...
        const sourceChart = document.getElementById(chartId)?.chartInstance;
        ...
        // Hide all charts in the background
        document.querySelectorAll(".charts").forEach(chart => {
            chart.style.visibility = "hidden";  // Hides without removing layout
        });
    ...
    document.querySelector(".overlay").addEventListener("click", function () {
        ...
        document.querySelectorAll(".charts.active").forEach(chart => {
    ...
    overlay.addEventListener("click", function (event) {
        if (!event.target.closest(".charts")) {
            document.querySelectorAll(".charts.active").forEach(activeChart => {
                activeChart.classList.remove("active");
            });
            overlay.style.display = "none";
